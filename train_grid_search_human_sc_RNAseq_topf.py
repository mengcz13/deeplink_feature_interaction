...
def train_grid_search():
    ...
    command_template = 'python train_pred.py --dataset_name human_sc_RNAseq_topf --nrep {nrep} --model_type MLP --layer_num {layer_num} --hidden_dims "{hidden_dims}" --activation {activation} --dropout {dropout} --batch_size {batch_size} --epochs 500 --lr {lr} --l1_weight {l1_weight} --l2_weight {l2_weight} --early_stopping_patience 50 --exptag human_sc_RNAseq_topf_param_search'

...
class Archipelago(Explainer):
    ...
    def explain(self, top_k=None, separate_effects=False):
        if (self.inter_sets is None) or (self.main_effects is None):
            ...
            self.inter_strengths = inter_strengths
    ...
    def archdetect_idv_strength(self):
        context = self.baseline
        insertion_target = self.input
        ...
        main_strengths = {}
        for i in idv_scores:
            effect_i = idv_scores[i] - context_score
            ell_i = np.abs(context[i[0]].item() - insertion_target[i[0]].item())
            if ell_i < 1e-6:
                dgs = 0
            else:
                dgs = np.square(effect_i / ell_i)
            main_strengths[i[0]] = dgs
        return main_strengths
